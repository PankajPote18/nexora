              ...
              {loading ? (
                ...
                filtered.map((item) => (
                  <tr key={item.id} className="hover:bg-[#1a2234] transition-colors">
                    ...
                    <td className="px-6 py-5 text-gray-300">{item.code}</td>
                    ...
                    <td className="px-6 py-5">
                      {item.image ? (
                        ...
                        <div className="w-10 h-10 bg-gray-800 rounded flex items-center justify-center text-xs text-gray-500">No Img</div>
                      )}
                    </td>
                    <td className="px-6 py-5">
          ...
          <div className="bg-[#1a2234] rounded-xl w-full max-w-xl max-h-[90vh] overflow-hidden shadow-2xl flex flex-col border border-gray-700/50">
            ...
            <div className="p-6 overflow-y-auto custom-scrollbar space-y-5">
              <div className="space-y-2">
                <label className="text-sm text-gray-300">Language Name <span className="text-red-500">*</span></label>
                <input
                  type="text"
                  value={formData.name}
              ...
              <div className="space-y-2">
                <label className="text-sm text-gray-300">Language Code <span className="text-red-500">*</span></label>
                <input
                  type="text"
                  value={formData.code}
              ...
              <div className="space-y-2">
                <label className="text-sm text-gray-300">Language Sorting</label>
                <input
                  type="number"
                  min="1"
                  ...
                  className="w-full bg-[#121826] border border-gray-700 text-white rounded px-4 py-2.5 outline-none focus:border-[#4aa5ff]"
                  placeholder="Enter language sorting order"
                />
              </div>

              <div className="space-y-2">
                <label className="text-sm text-gray-300">Flag Icon Image</label>
                <div className="flex items-center border border-gray-700 rounded bg-[#121826] overflow-hidden">
                  <label className="cursor-pointer bg-[#2a3449] hover:bg-[#3a4560] text-white px-4 py-2.5 text-sm font-medium transition-colors border-r border-gray-700 shrink-0">
                    Choose File
                    <input type="file" accept="image/*" className="hidden" onChange={handleImageChange} />
                  </label>
                  <span className="px-4 text-sm text-[#00E5FF] truncate">
                    {formData.image ? (formData.image.name || 'File selected') : 'No file chosen'}
                  </span>
                </div>
                <p className="text-xs text-gray-500 pt-1">
                  Optional: Upload language flag image (max 2MB)
                </p>
                {formData.imagePreview && (
                  <div className="mt-3">
                    <img src={formData.imagePreview} alt="Preview" className="w-16 h-16 object-contain rounded bg-[#121826] border border-gray-700" />
                  </div>
                )}
              </div>
            </div>
      ...
      {deleteId && (
        <div className="fixed inset-0 z-[110] flex items-center justify-center bg-black/80 backdrop-blur-sm p-4">
          <div className="w-full max-w-sm bg-[#1a2234] border border-gray-700 rounded-xl p-6 shadow-2xl">
            <h3 className="text-lg font-medium text-white mb-2">Delete Language</h3>
