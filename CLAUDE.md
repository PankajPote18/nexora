...
**TEMPORARY — PayU Test Mode section (`PlansPage.jsx`)**: a dev-only "PayU Test Mode" block was added to the bottom of the real `/plans` page (clearly marked, dashed border, separate `test*` state namespace from the real checkout flow) so the site owner can trigger a real ₹1 UPI Intent transaction from their own phone against the PayU **test** sandbox, using PayU's official UPI Intent Simulator Android app. It reuses `paymentsApi.create`/`paymentsApi.getStatus` and the existing `POST /api/subscription-plans` endpoint unmodified — no new backend code. On first use it creates (or reuses, by exact name match) a `SubscriptionPlan` row named `PayU Test Plan (Do Not Purchase)` (`discounted_price: 1.00`), since `createPayment` intentionally never accepts a client-supplied amount. That plan is filtered out of the real customer-facing plan list by name in `PlansPage.jsx` (`plans.filter(p => p.name !== TEST_PLAN_NAME)` in two places — the render list and the default-selection logic). **Remove the whole block (search `TEMPORARY` in `PlansPage.jsx`) and delete the `PayU Test Plan (Do Not Purchase)` row (via Admin → Subscriptions) once sandbox testing is done.**
...
- **`subscription_plans` table silently mangles non-ASCII characters** (verified directly: `POST /api/subscription-plans` with `name: "PayU Test (₹1)"` came back as `"PayU Test (?1)"`) — the same class of charset/collation issue `fix-collation.js` already fixed for `hero_banners`/`movies` (likely not utf8mb4). Any code writing ₹/emoji/non-ASCII into `SubscriptionPlan.name` (or probably other string columns on this table) needs to either avoid non-ASCII in that column or fix the table's collation first — don't assume text round-trips correctly on this table like it does on the ones `fix-collation.js` already touched.
